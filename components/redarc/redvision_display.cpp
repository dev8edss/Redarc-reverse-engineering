...
void RedvisionDisplayComponent::handle_can_frame(uint32_t can_id, const std::vector<uint8_t> &data) {
  ...
  switch (redarc_common::rvc_dgn(can_id)) {
    case 0x1F280UL: {
      if (this->last_1f280_ms_ != 0 && now - this->last_1f280_ms_ < this->filter_interval_ms_) return;
      this->last_1f280_ms_ = now;
      const uint16_t raw_batt = redarc_common::u16_le(data, 0);
      const uint16_t raw_load = redarc_common::u16_le(data, 4);
      if (this->battery_current_display_sensor_ != nullptr)
        this->battery_current_display_sensor_->publish_state(redarc_common::current_display_16_centered(raw_batt));
      if (this->device_current_display_sensor_ != nullptr)
        this->device_current_display_sensor_->publish_state(redarc_common::current_display_16_centered(raw_load));
      return;
    }

    case 0x1F282UL: {
      if (this->last_1f282_ms_ != 0 && now - this->last_1f282_ms_ < this->filter_interval_ms_) return;
      this->last_1f282_ms_ = now;
      const uint16_t raw_mgr = redarc_common::u16_le(data, 6);
      if (this->manager_output_current_display_sensor_ != nullptr)
        this->manager_output_current_display_sensor_->publish_state(redarc_common::current_display_16_centered(raw_mgr));
      return;
    }
